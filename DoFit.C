...
double fitshape( double *x, double *par)
{
...
//extern ShapeFcn(double *x, double *par);

void DoFit(  TGraph *graph, TF1 *fitFcn, Double_t xmin, Double_t xmax) {
 //Bevington Exercise by Peter Malzacher, modified by Rene Brun
   ...
   // respond well.
   fitFcn->SetParameters(1.89,28.057);
   fitFcn->Draw("same");
   //TGraph *graph = new TGraph(histo);
   //fitFcn->GetRange(xmin,xmax);
   //graph->Fit(fitFcn,"0","",26.,30.);
   graph->Fit(fitFcn,"0","",xmin,xmax);
   ...
   TF1 *signalFcn = new TF1("signalFcn",fitshape,25.0,29.0,4);
